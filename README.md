...
- Rewrote app in TypeScript with organized module structure: `src/lib/`, `src/odpt/`, `src/ui/`, and `src/trainboard.ts`.
...
- Smart interval management with tickManager for efficient refresh scheduling.
- Lit-based web components for modular, reusable UI elements.
- Unit tests (Vitest) for parsing utilities, routing, geolocation, rendering, visibility management, and more are included.
...
- `src/` — TypeScript sources organized into subdirectories:
  - `src/lib/` — Core utilities (cache, config, location, tickManager, visibilityManager, utils)
  - `src/odpt/` — ODPT API client and data loaders (api, dataLoaders, types)
  - `src/ui/` — UI rendering and components (renderBoard, settings, components/)
  - `src/routing.ts` — URL routing and navigation
  - `src/trainboard.ts` — Main application entry point
