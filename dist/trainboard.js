...
let ODPT_API_KEY = null; // loaded from ./config.json at runtime
...
async function fetchRailwayStations() {
    ...
    try {
        ...
        STATION_CONFIGS = data
            .map((station) => {
            const stationNameJa = station['dc:title'];
            const stationCode = station['odpt:stationCode'] || '';
            return { name: `${stationNameJa} (${stationCode})`, uri: station['owl:sameAs'] };
        })
...
async function renderBoard() {
    ...
    if (!stationConfig) {
        ...
        if (hdr)
            hdr.textContent = 'エラー: 駅が選択されていません';
        return;
    }
    const hdr = safeGetElement('station-header');
    if (hdr)
        hdr.textContent = stationConfig.name;
    ...
    if (inContainer)
        inContainer.innerHTML = loadingHtml;
    if (outContainer)
        outContainer.innerHTML = loadingHtml;
    const inHeader = safeGetElement('direction-inbound-header');
    const outHeader = safeGetElement('direction-outbound-header');
    if (inHeader)
        inHeader.textContent = INBOUND_FRIENDLY_NAME_JA;
    if (outHeader)
        outHeader.textContent = OUTBOUND_FRIENDLY_NAME_JA;
    ...
    statusIntervalId = window.setInterval(fetchStatus, 300000);
...
async function initializeBoard() {
    ...
    if (!ODPT_API_KEY) {
        const hdr = safeGetElement('station-header');
        if (hdr)
            hdr.textContent = '設定エラー: config.json に ODPT_API_KEY を設定してください';
